...
export default function HeroSection() {
  ...
  return (
    <section ref={ref} className="grid grid-cols-1 lg:grid-cols-12 gap-gutter items-start">
      {/* Left: Hero Copy & Identity */}
      <div className="lg:col-span-5 flex flex-col gap-md pt-lg relative z-10">
        {/* Washi Tape Identity */}
        <div className="reveal reveal-delay-1 self-start bg-primary/20 text-primary px-md py-sm rotate-[-3deg] shadow-sm transform hover:rotate-0 transition-transform cursor-default inline-flex items-center gap-sm animate-wiggle">
          <span className="material-symbols-outlined text-[20px]">school</span>
          <span className="font-label-caps text-label-caps tracking-widest uppercase">IX-2 • Solidaritas Tanpa Batas</span>
        </div>
        <h1 className="reveal reveal-delay-2 font-display-lg-mobile md:font-display-lg text-display-lg-mobile md:text-display-lg text-primary mt-md leading-tight">
          Ingat masa-masa indah di kelas IX-2?
        </h1>
        <p className="reveal reveal-delay-3 font-body-lg text-body-lg text-on-surface-variant mt-sm">
        ...
        {/* Sticker Button */}
        <button className="reveal reveal-delay-4 mt-md self-start bg-secondary-container text-on-secondary-container font-label-caps text-label-caps px-xl py-sm border-2 border-primary shadow-[4px_4px_0px_0px_rgba(3,31,65,1)] hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-[2px_2px_0px_0px_rgba(3,31,65,1)] transition-all active:shadow-none active:translate-x-[4px] active:translate-y-[4px]">
          MULAI JELAJAH
        </button>
      </div>

      {/* Right: Big Polaroid Photo */}
      <div className="reveal-right lg:col-span-7 relative mt-xl lg:mt-0">
        {/* Decorative element behind photo */}
        <div className="absolute -top-lg -right-md w-32 h-32 bg-tertiary-fixed-dim rotate-12 z-0 hidden md:block animate-spin-slow opacity-70"></div>
        {/* The Polaroid Container */}
        <div className="animate-float-y bg-surface-container-lowest p-[8px] pb-[32px] md:pb-[48px] shadow-[6px_6px_0px_0px_rgba(29,53,87,1)] rotate-[2deg] hover:rotate-0 transition-transform duration-500 relative z-10 border border-outline-variant/30 group">
          {/* Top tape */}
